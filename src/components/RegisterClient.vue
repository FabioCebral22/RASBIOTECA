    ...
    <main>
      ...
      <div class="right-side">
        <form @submit.prevent="registerClient">
          ...
          <span class="error" v-if="formError && !validateName()">El nombre debe contener solo letras</span>

          <label for="nickname">Nickname</label>
          <input type="text" v-model="clientNickname" placeholder="Introduce tu nombre de usuario" required />

          <label for="email">Email</label>
          <input type="email" v-model="clientEmail" placeholder="Introduce tu correo" required />

          <label for="password">Password</label>
          <input type="password" v-model="clientPassword" placeholder="Introduce tu contraseña" required />
          ...
          <span class="error" v-if="formError && clientPassword !== confirmPassword">Las contraseñas no coinciden</span>
...
export default {
  ...
  methods: {
    validateName() {
      return /^[a-zA-Z\s]+$/.test(this.clientName);
    },
    validatePassword() {
      return /^(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{8,}$/.test(this.clientPassword);
    },
    async registerClient() {
      if (!this.validateName() || !this.validatePassword() || this.clientPassword !== this.confirmPassword) {
        this.formError = true;
        return;
      }
      try {
        const response = await fetch('http://raul-argemi-7e6.alwaysdata.net/api/clients', {
          method: 'POST',
          headers: {
          ...
          body: JSON.stringify({
            ...
            client_nickname: this.clientNickname,
